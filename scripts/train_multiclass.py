"""Training script for multiclass protein classification.

Supports CellAtlas and BioSR datasets, DenseNet and ResNet architectures.

Examples
--------
# CellAtlas with DenseNet121 (default)
python scripts/train_multiclass.py --num_crops 4

# CellAtlas with ResNet18
python scripts/train_multiclass.py --arch resnet18 --crop_size 256 --num_crops 4

# BioSR
python scripts/train_multiclass.py --dataset BioSR \
    --labels F-actin Microtubules CCPs ER --num_crops 4
"""
import argparse
import os
import socket
...
parser = argparse.ArgumentParser(
    description="Train a multiclass protein classifier.",
...
# --- model config ---
num_classes = len(args.labels) + 3  # +3 for reference channels: Nucleus, Microtubules, ER
...
exp_name = f"{arch_name}_{args.dataset}_{num_classes}Cl_{args.labels[0]}"
...
print(f"Mode               : multiclass ({num_classes} classes)")
